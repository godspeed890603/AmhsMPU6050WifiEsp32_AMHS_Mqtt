...
bool iotSqliteDB::sinitialSqliteDB(String mpu_id) {
  ...
  char fullDBFileName[60];
  Serial.println("mpu_id.c_str():");
  Serial.println(mpu_id.c_str());
  sprintf(fullDBFileName, "/spiffs/%s.db", mpu_id.c_str());
  Serial.println("fullDBFileName:");
  Serial.println(fullDBFileName);
  Serial.println("fullDBFileName:end");
  ...
  if (sdb_open(fullDBFileName, &db1) == 0) {
    Serial.println("sdb_open.OK");
    return true;
  } else {
    Serial.println("sdb_open.NG");
    return false;
  }
}
...
bool iotSqliteDB::createTable() {
  ...
  Serial.println("=======================");
  Serial.print("createTable rc=");
  Serial.println(rc);

  if (rc == 0) {
    Serial.println("CREATE TABLE OK..");
    return true;
  }

  if (rc != 0) {
    sqlite3_close(db1);
    Serial.println("CREATE TABLE FAIL..");
    return false;
  }
}
...
int iotSqliteDB::checkAccOffset() {
  ...
  if (db1 == NULL) {
    Serial.println("Database connection is NULL.");
    return -99999;
  } else {
    Serial.println("Database connection is OK.");
  }

  String sql = "SELECT COUNT(*) FROM ACCELOFFSET";

  // rc =
  //     sqlite3_prepare_v2(db1, "SELECT COUNT(*) FROM ACCELOFFSET", -1, &stmt,
  //     0);
  rc = sqlite3_prepare_v2(db1, sql.c_str(), -1, &stmt, 0);
  if (rc == SQLITE_OK) {
    ...
    Serial.println(
        sqlite3_errmsg(db1));  // Print the detailed SQLite error message
    return -99999;
  }
}
...
bool iotSqliteDB::ReadAccOffset(float *accelOffsetX, float *accelOffsetY,
  ...
  // 執行SELECT查詢
  sqlite3_stmt *stmt;
  String sql = "SELECT * FROM ACCELOFFSET";
  rc = sqlite3_prepare_v2(db1, sql.c_str(), -1, &stmt, 0);
